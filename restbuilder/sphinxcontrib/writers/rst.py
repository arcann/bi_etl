...
class Cell(object):
    def __init__(self, text, cols_spanned):
        self.text = text
        self.cols_spanned = cols_spanned


class Row(object):
    def __init__(self, columns, max_lines=1):
...
class RstTranslator(TextTranslator):
    ...
    @staticmethod
    def escape_text(text, escape_chars=None):
        if escape_chars is None:
            escape_chars = RstTranslator.escape_chars
        for escape_char in escape_chars:
            # TODO: Make sure it's not already escaped
    ...
    def visit_document(self, node):
        self.new_state(0)
        self.add_text('.. Auto-generated via Sphinx. Do not edit manually.\n\n')
    ...
    def visit_desc_parameter(self, node):
        # TODO: Check overall length of parameter list and convert to multi-line if too long
        if not self.first_param:
            self.add_text(', ')
        else:
            self.first_param = False
        text = node.astext()
        text = RstTranslator.escape_text(text)
        self.add_text(text)
        raise nodes.SkipNode
    ...
    def visit_desc_content(self, node):
        #TODO: Don't output blank line, so that we use the definition list model and not the quote block
    ...
    def visit_entry(self, node):
        ...
        if 'morerows' in node:
            raise NotImplementedError('Column or row spanning cells are '
                                      'not implemented. See {}'.format(node))
        self.new_state(0)
    ...
    def depart_entry(self, node):
        ...
        text_list = []
        state_values = []
        for x in self.states.pop():
            state_values.append(x)
            text_list.append(self.nl.join(x[1]))
        text = self.nl.join(text_list)
        cols_spanned = 1
        if 'morecols' in node:
            cols_spanned += node['morecols']
        self.stateindent.pop()
        self.table[-1].append(Cell(text=text, cols_spanned=cols_spanned))
    ...
    def depart_table(self, node):
        ...
        for line in lines:
            if line == 'sep':
                ...
                col_num = 0
                for cell in line:
                    par = self.wrap(cell.text, width=colwidths[col_num])
                    if par:
                        maxwidth = max(map(len, par))
                    else:
                        maxwidth = 0
                    realwidths[col_num] = max(realwidths[col_num], maxwidth)
                    formatted_cell = Cell(par, cell.cols_spanned)
                    cells.append(formatted_cell)
                    col_num += cell.cols_spanned

                    lines = len(par)
                    if lines > row.max_lines:
                        row.max_lines = lines

                fmted_rows.append(row)
        ...
        def writerow(row):
            # row is now Row with max_lines and cells as a list of Cells which contain lists of lines
            # row WAS a list of lists of lines
            # lines = zip(*row)
            # for line in lines:
            for line_num in range(row.max_lines):
                out = ['|']
                col_num = 0
                for cell in row.columns:
                    width = realwidths[col_num]
                    for _ in range(cell.cols_spanned-1):
                        col_num += 1
                        width += realwidths[col_num]
                    if len(cell.text) > line_num:
                        out.append(' ' + cell.text[line_num].ljust(width + 1))
                    else:
                        out.append(' ' * (width + 2))
                    out.append('|')
                    col_num += 1
    ...
    def visit_reference(self, node):
        ...
        if 'refuri' not in node:
            if 'name' in node:
                self.add_text('`{}`_'.format(node['name']))
            elif 'reftitle' in node:
                self.add_text('`{}`_'.format(node['reftitle']))
            else:
                self.add_text('{}'.format(node))
            raise nodes.SkipNode
        elif 'internal' not in node:
            if 'name' in node:
                self.add_text('`{} <{}>`_'.format(node['name'], node['refuri']))
            else:
                self.add_text('`{} <{}>`_'.format(node['refuri'], node['refuri']))
            raise nodes.SkipNode
        elif 'reftitle' in node:
    ...
    def visit_Text(self, node):
        ...
        if self.escaping:
            if self.table:
                escape_chars = self.table_escape_chars
            else:
                escape_chars = self.escape_chars
            text = RstTranslator.escape_text(text, escape_chars)
        self.add_text(text)
    ...
    def visit_problematic(self, node):
        if node.rawsource.startswith(':paramref:'):
            self.add_text(node.rawsource[10:])
            raise nodes.SkipNode
        if node.rawsource[0] not in self.escape_chars:
            warnings.warn('Problematic char {} not in escape_chars'.format(node.rawsource) )
            self.add_text('\\')
