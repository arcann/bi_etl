...
class TestHstTable(unittest.TestCase):
    def setUp(self):
        ...
        self.config = build_config(db_config, self.tmp)
        self.task = ETLTask(config=self.config)
        self.mock_database = self.task.get_database_metadata(db_config)
    ...
    def testInsertAndIterate(self):
        ...
        rows_to_insert = 10
        source_compontent = DummyETLComponent(config=self.config)
        ...
        self.mock_database.execute(f'DROP TABLE {tbl_name}')
