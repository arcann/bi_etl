...
class TestRow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.row1 = RowCaseInsensitive(self.source1)
        
        # Make a SQLAlchemy type of row (RowProxy)
        self.source2 = self._make_row_from_dict(self.source1)
        self.row2 = RowCaseInsensitive(self.source2, name='my_row_name')
        ...
        self.row3 = RowCaseInsensitive(list(zip(self.keys, self.values)), parent=self.row3_parent)
    ...
    def test_columns_in_order(self):
        ...
        expected_keys = [k.lower() for k in self.keys]
    ...
    def _test_create_performance(self):
        ...
        timer.reset()
        for _ in range(self.create_performance_iterations):
            row_lst.append( RowCaseInsensitive(self.source1) )
        timer.stop()
