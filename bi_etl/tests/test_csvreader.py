...
class Test(unittest.TestCase):
    ...
    def testUTF8_CSV_with_header_where(self):
        ...
        quotedSrcFile = repr(srcFile)
        #print(quotedSrcFile)
        self.maxDiff = None  
        ## Test passing opened file  
        with open(srcFile, mode='rt', encoding='utf-8') as srcFileData: 
            with CSVReader(self.task, srcFileData) as src:
                self.assertEqual(src.column_names, ['str','int','float','date','unicode'])
                expectedRepr="CSVReader(task={task},logical_name={logical_name},filedata=<_io.TextIOWrapper name={file} mode='rt' encoding='utf-8'>,primary_key=None,column_names=['str', 'int', 'float', 'date', 'unicode'])"
                expectedRepr = expectedRepr.format(task=self.task, logical_name=logical_name, file=quotedSrcFile)
                self.assertEqual(repr(src), expectedRepr)
                src_iter = src.where({'str':'Bob'})
                row = next(src_iter)
    ...
    def testUTF8_tab_no_header(self):
        srcFile = os.path.join(self.test_files_path, 'utf8_no_header.tab')
        logical_name = os.path.basename(srcFile)
        #print(srcFile)
        quotedSrcFile = repr(srcFile)
        #print(quotedSrcFile)
    ...
    def testUTF8_pipe_utf8_with_header_mixed_row_len(self):
        srcFile = os.path.join(self.test_files_path, 'utf8_with_header_mixed_row_len.pipe')
        logical_name = os.path.basename(srcFile)
        #print(srcFile)
        quotedSrcFile = repr(srcFile)
        #print(quotedSrcFile)
        ...
        with CSVReader(self.task, srcFile, encoding='utf-8') as src:
            ...
            expectedRepr = "CSVReader(task={task},logical_name={logical_name},filedata=<_io.TextIOWrapper name={file} mode='rt' encoding='utf-8'>,primary_key=None,column_names=['str', 'col2', 'col3'])"
            expectedRepr = expectedRepr.format(task=self.task, logical_name=logical_name, file=quotedSrcFile)
            self.assertEqual(repr(src), expectedRepr)
            src_iter = iter(src)
